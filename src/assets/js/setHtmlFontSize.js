...
(function (){
	...
	self.widthProportion = function() {
		var p = (document.body&&document.body.clientWidth||document.getElementsByTagName("html")[0].offsetWidth)/self.width; return p<minUnit?minUnit:p;
	};
	self.changePage = function(){
		self.val = self.widthProportion()*self.fontSize;
		if (self.val > constMaxHtmlFontSize) { self.val = constMaxHtmlFontSize}
		document.getElementsByTagName("html")[0].setAttribute("style","font-size:"+self.val+"px !important");
	}
	self.changePage();
	window.addEventListener("resize",function(){self.changePage();},false);
